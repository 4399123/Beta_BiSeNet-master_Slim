...
class FastEfficientBiSeNet_CPUBone_Nano_Reparam(nn.Module):
    ...
    def __init__(self, n_classes, aux_mode='train', use_fp16=False, img_size=(512, 512)):
        ...
        self.register_buffer('pixel_mean', torch.tensor([120.0, 114.0, 104.0]).view(1, 3, 1, 1))
        self.register_buffer('pixel_std',  torch.tensor([ 70.0,  69.0,  73.0]).view(1, 3, 1, 1))
    ...
    def forward(self, x):
        with autocast(enabled=self.use_fp16):
            ...
            if (self.aux_mode == 'pred'):
                ...
                # 归一化：(x - mean) / std，mean/std 已在 __init__ 中注册为 buffer
