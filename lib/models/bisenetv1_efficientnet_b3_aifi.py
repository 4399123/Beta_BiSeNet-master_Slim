...
class AIFI(nn.Module):
    ...
    def forward(self, x):
        """
        输入:
            x: 展平后的特征序列，形状 (batch_size, seq_len, d_model)
            pos_embed: 位置编码，形状 (seq_len, d_model)
        输出:
            处理后的特征序列，形状 (batch_size, seq_len, d_model)
        """
        batch_size, C, H, W = x.shape
        x = x.view(batch_size, C, -1).permute(0, 2, 1)  # 形状变为 (batch_size, H*W, C)
        ...
        # 将输出x从 (batch_size, H*W, C) 转换回 (batch_size, C, H, W)
        x = x.permute(0, 2, 1).view(batch_size, C, H, W)
...
class BiSeNetV1_EfficientNet_B3_AIFI(nn.Module):

    def __init__(self, n_classes, aux_mode='train',use_fp16=False, *args, **kwargs):
        super(BiSeNetV1_EfficientNet_B3_AIFI, self).__init__()
        self.cp = ContextPath()
        self.sp = SpatialPath()
...
if __name__ == "__main__":
    net = BiSeNetV1_EfficientNet_B3_AIFI(19)
    net.eval()
