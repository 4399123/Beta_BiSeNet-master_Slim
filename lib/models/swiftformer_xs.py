...
class SwiftFormer_XS(nn.Module):
    def __init__(self):
        super(SwiftFormer_XS, self).__init__()
        self.out_indices = [1, 2,3]
        self.selected_feature_extractor = timm.create_model('swiftformer_xs.dist_in1k', features_only=True, out_indices=self.out_indices,pretrained=False)
        try:
            load_checkpoint(self.selected_feature_extractor, '../lib/premodels/swiftformer_xs.pth')
        except:
            load_checkpoint(self.selected_feature_extractor, '../premodels/swiftformer_xs.pth')
