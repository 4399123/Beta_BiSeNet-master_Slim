...
class ShVit_S1(nn.Module):
    def __init__(self):
        super(ShVit_S1, self).__init__()
        self.out_indices = [0,1, 2]
        self.selected_feature_extractor = timm.create_model('shvit_s1.in1k', features_only=True, out_indices=self.out_indices,pretrained=False)
        try:
            load_checkpoint(self.selected_feature_extractor, '../lib/premodels/shvit_s1.pth')
        except:
            load_checkpoint(self.selected_feature_extractor, '../premodels/shvit_s1.pth')
