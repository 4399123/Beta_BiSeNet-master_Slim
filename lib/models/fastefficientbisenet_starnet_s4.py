...
from .starnet_s4 import SatrNet_S4
...
class FastEfficientBiSeNet_SatrNet_S4(nn.Module):
    def __init__(self, n_classes, aux_mode='train', use_fp16=False, img_size=(512, 512)):
        """
        img_size: 用于计算 SPPM 静态池化参数，务必与实际输入一致。
        """
        super(FastEfficientBiSeNet_SatrNet_S4, self).__init__()
        self.use_fp16 = use_fp16
        self.aux_mode = aux_mode
        self.img_size = img_size

        # 1. 骨干网络
        self.backbone = SatrNet_S4()

        # 通道定义 (需根据实际 Backbone 输出调整)
        self.c3_chan = 64  # Stride 8
        self.c4_chan = 128  # Stride 16
        self.c5_chan = 256  # Stride 32
...
if __name__ == "__main__":
    ...
    try:
        ...
        net = FastEfficientBiSeNet_SatrNet_S4(n_classes=n_classes, aux_mode='train', img_size=(img_height, img_width))
        net.train()
