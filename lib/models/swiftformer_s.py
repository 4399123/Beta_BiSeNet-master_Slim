...
class SwiftFormer_S(nn.Module):
    def __init__(self):
        super(SwiftFormer_S, self).__init__()
        self.out_indices = [1, 2,3]
        self.selected_feature_extractor = timm.create_model('swiftformer_s.dist_in1k', features_only=True, out_indices=self.out_indices,pretrained=False)
        try:
            load_checkpoint(self.selected_feature_extractor, '../lib/premodels/swiftformer_s.pth')
        except:
            load_checkpoint(self.selected_feature_extractor, '../premodels/swiftformer_s.pth')
