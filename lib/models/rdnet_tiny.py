...
class RDNet_Tiny(nn.Module):
    def __init__(self):
        super(RDNet_Tiny, self).__init__()
        self.out_indices = [1, 2,3]
        self.selected_feature_extractor = timm.create_model('naver-ai/rdnet_tiny.nv_in1k', features_only=True, out_indices=self.out_indices,pretrained=False)

        load_checkpoint(self.selected_feature_extractor, '../lib/premodels/rdnet_tiny.pth')
        # load_checkpoint(self.selected_feature_extractor, '../premodels/rdnet_tiny.pth')
