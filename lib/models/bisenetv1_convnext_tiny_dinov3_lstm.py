...
from .convnext_tiny_dinov3 import ConvNeXt_Tiny_DinoV3
...
class ContextPath(nn.Module):
    def __init__(self, *args, **kwargs):
        ...
        self.resnet = ConvNeXt_Tiny_DinoV3()
        ...
        self.conv_avg = ConvBNReLU(768, 256, ks=1, stride=1, padding=0)
...
# =============================================================================
#  新增模块: Bi-directional LSTM Enhancement Block
# =============================================================================
class BiLSTMEnhancementBlock(nn.Module):
    def __init__(self, in_chan, num_layers=1):
        super().__init__()
        self.in_chan = in_chan

        # LSTM的隐藏维度设置为通道数的一半，因为双向LSTM会拼接两个方向的输出
        # hidden_size * 2 = in_chan
        hidden_size = in_chan // 2
    ...
    def forward(self, x):
        ...
        # 将图像特征转换为序列: [B, C, H, W] -> [B, H*W, C]
        x_seq = x.flatten(2).transpose(1, 2)

        # Layer Normalization (有助于稳定RNN训练)
        x_seq = self.norm(x_seq)

        # 通过LSTM处理序列
        # 输出的形状是 [B, H*W, hidden_size*2], 也就是 [B, H*W, C]
        lstm_out, _ = self.lstm(x_seq)

        # 将序列转回图像特征: [B, H*W, C] -> [B, C, H, W]
        x_enhanced = lstm_out.transpose(1, 2).reshape(B, C, H, W)

        # 添加残差连接
        return x_enhanced + x_residual

class BiSeNetV1_ConvNeXt_Tiny_DinoV3_LSTM(nn.Module):

    def __init__(self, n_classes, aux_mode='train',use_fp16=False, *args, **kwargs):
        super(BiSeNetV1_ConvNeXt_Tiny_DinoV3_LSTM, self).__init__()
        self.cp = ContextPath()
        self.sp = SpatialPath()
        self.ffm = FeatureFusionModule(512, 512)

        # === 在FFM后加入新的Bi-LSTM增强模块 ===
        self.lstm_enhancement = BiLSTMEnhancementBlock(in_chan=512)
        # ======================================
    ...
    def forward(self, x):
        with autocast(enabled=self.use_fp16):
            ...
            # === 使用LSTM模块增强特征 ===
            feat_enhanced = self.lstm_enhancement(feat_fuse)
            # ============================

            # === 使用增强后的特征进行输出 ===
            feat_out = self.conv_out(feat_enhanced)
            # ============================
            if self.aux_mode == 'train':
...
if __name__ == "__main__":
    net = BiSeNetV1_ConvNeXt_Tiny_DinoV3_LSTM(19)
    net.eval()
