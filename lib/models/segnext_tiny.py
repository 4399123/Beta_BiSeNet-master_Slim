...
from .segnext_bricks  import DepthwiseConv,DownSampling,DropPath
...
class MSCAN(nn.Module):
    ...
    def init_weight(self):
        ...
        checkpoint = torch.load('../lib/premodels/mscan_t.pth')
        # checkpoint = torch.load('../premodels/mscan_t.pth')
        checkpoint["state_dict"] = {
            tp[0]: tp[-1]
            for tp in zip(self.state_dict().keys(), checkpoint["state_dict"].values())
        }
...
if __name__ == "__main__":
    net = SegNeXt_Tiny(n_classes=9)
    net.eval()
    net.cuda()
    in_ten = torch.randn(2, 3, 224, 224).to(device="cuda")
    aa = net(in_ten)
    print(aa.shape)