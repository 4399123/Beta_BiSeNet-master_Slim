...
class ContextPath(nn.Module):
    def __init__(self, *args, **kwargs):
        ...
        self.resnet = RepGhostNet_080()
        self.arm16 = AttentionRefinementModule(64, 256)
        self.arm32 = AttentionRefinementModule(128, 256)
        self.conv_head32 = ConvBNReLU(256, 256, ks=3, stride=1, padding=1)
        self.conv_head16 = ConvBNReLU(256, 256, ks=3, stride=1, padding=1)
        self.conv_avg = ConvBNReLU(128, 256, ks=1, stride=1, padding=0)
...
class BiSeNetV1_RepGhostNet_080(nn.Module):

    def __init__(self, n_classes, aux_mode='train',use_fp16=False, *args, **kwargs):
        super(BiSeNetV1_RepGhostNet_080, self).__init__()
        self.cp = ContextPath()
        self.sp = SpatialPath()
        ...
        if self.aux_mode == 'train':
            self.conv_out16 = BiSeNetOutput(256, 256, n_classes, up_factor=8)
            self.conv_out32 = BiSeNetOutput(256, 256, n_classes, up_factor=16)
        # self.init_weight()
...
if __name__ == "__main__":
    net = BiSeNetV1_RepGhostNet_080(19)
    net.eval()
