...
class MambaOut_Tiny(nn.Module):
    def __init__(self):
        super(MambaOut_Tiny, self).__init__()
        self.out_indices = [1, 2,3]
        self.selected_feature_extractor = timm.create_model('mambaout_tiny.in1k', features_only=True, out_indices=self.out_indices,pretrained=False)
        try:
            load_checkpoint(self.selected_feature_extractor, '../lib/premodels/mambaout_tiny.pth')
        except:
            load_checkpoint(self.selected_feature_extractor, '../premodels/mambaout_tiny.pth')
