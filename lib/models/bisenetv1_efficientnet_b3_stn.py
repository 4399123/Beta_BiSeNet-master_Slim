...
class BiSeNetV1_EfficientNet_B3_STN(nn.Module):

    def __init__(self, n_classes, aux_mode='train',use_fp16=False, *args, **kwargs):
        super(BiSeNetV1_EfficientNet_B3_STN, self).__init__()
        self.cp = ContextPath()
        self.sp = SpatialPath()
        ...
        self.fc_loc_conv1=nn.Conv2d(in_channels=10,out_channels=32,kernel_size=(1,1),padding=(0,0))
        self.fc_loc_conv2 =nn.Conv2d(in_channels=32,out_channels=6,kernel_size=(1,1),padding=(0,0))
    # Spatial transformer network forward function
    def stn(self, x):
        xs = self.localization(x)
        xs=self.fc_loc_conv1(xs)
        xs=F.relu(xs)
        xs=self.fc_loc_conv2(xs)
        theta = torch.mean(xs, dim=(2, 3), keepdim=True)
        theta = torch.flatten(theta, 1)
...
if __name__ == "__main__":
    net = BiSeNetV1_EfficientNet_B3_STN(19)
    net.eval()
