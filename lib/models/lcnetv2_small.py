...
class LCNet_Small(nn.Module):
    def __init__(self):
        super(LCNet_Small, self).__init__()
        self.out_indices = [1, 2,3]
        self.selected_feature_extractor = timm.create_model('lcnetv2_small.paddle_in1k', features_only=True, out_indices=self.out_indices,pretrained=False)
        try:
            load_checkpoint(self.selected_feature_extractor, '../lib/premodels/lcnetv2_small.pth')
        except FileNotFoundError:
            load_checkpoint(self.selected_feature_extractor, '../premodels/lcnetv2_small.pth')
