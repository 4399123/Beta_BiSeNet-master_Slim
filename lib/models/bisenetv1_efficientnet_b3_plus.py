...
class FeatureFusionModule(nn.Module):
    ...
    def forward(self, fsp, fcp):
        atten = torch.mean(fcp, dim=(2, 3), keepdim=True)
        atten = self.conv(atten)
        atten = self.bn(atten)
        atten = atten.sigmoid()
        feat_atten = torch.mul(fsp, atten)
        return feat_atten



class BiSeNetV1_EfficientNet_B3_Plus(nn.Module):

    def __init__(self, n_classes, aux_mode='train',use_fp16=False, *args, **kwargs):
        super(BiSeNetV1_EfficientNet_B3_Plus, self).__init__()
        self.cp = ContextPath()
        self.sp = SpatialPath()
...
if __name__ == "__main__":
    net = BiSeNetV1_EfficientNet_B3_Plus(19)
    net.eval()
